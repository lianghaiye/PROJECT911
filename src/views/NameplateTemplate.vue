    ...
    <template v-if="currentView === 'list'">
      <div class="page-card">
        ...
        <div class="sub-tabs">
          ...
          <router-link to="/base-config/product-template" class="sub-tab">配置产品信息更新模板</router-link>
          <router-link to="/base-config/nameplate-template" class="sub-tab active">配置铭牌模板</router-link>
        </div>
          ...
          <table class="data-table">
            <thead>
              <tr>
                ...
                <th width="160" class="sticky-col">操作</th>
              </tr>
            </thead>
            <tbody>
      ...
      templates: [
        {
          ...
          fields: [
            { code: 'product_name', name: '产品名称', type: 'text', format: '', required: true, placeholder: '请输入产品名称', charLimit: 100, options: [], defaultValue: '' },
            { code: 'product_model', name: '产品型号', type: 'text', format: '', required: true, placeholder: '请输入产品型号', charLimit: 50, options: [], defaultValue: '' },
            { code: 'factory_code', name: '出厂编码', type: 'text', format: '', required: true, placeholder: '请输入出厂编码', charLimit: 40, options: [], defaultValue: '' },
            { code: 'factory_date', name: '出厂日期', type: 'date', format: 'yyyy-MM-dd', required: true, placeholder: '请选择日期', charLimit: null, options: [], defaultValue: '' },
            { code: 'serial_no', name: '产品序列号', type: 'text', format: '', required: true, placeholder: '请输入产品序列号', charLimit: 60, options: [], defaultValue: '' }
          ]
        },
        {
          id: 'npl_usr_001',
          ...
          updater: '张三',
          updateTime: '2025-04-15 16:30:00',
          fields: [
            { code: 'product_name', name: '产品名称', type: 'text', format: '', required: true, placeholder: '请输入产品名称', charLimit: 100, options: [], defaultValue: '' },
            { code: 'product_model', name: '产品型号', type: 'text', format: '', required: true, placeholder: '请输入产品型号', charLimit: 50, options: [], defaultValue: '' },
            { code: 'factory_code', name: '出厂编码', type: 'text', format: '', required: true, placeholder: '请输入出厂编码', charLimit: 40, options: [], defaultValue: '' },
            { code: 'rated_power', name: '额定功率(kW)', type: 'number', format: '0.0', required: false, placeholder: '请输入额定功率', charLimit: null, options: [], defaultValue: '' }
          ]
        }
      ],
      // 新增/编辑模板
  ...
  methods: {
    ...
    saveTemplate() {
      ...
      if (this.editingTemplate) {
        ...
        const id = 'npl_' + Date.now();
        this.templates.push({
          id,
          code: 'NPL-USR-' + String(this.templates.length + 1).padStart(3, '0'),
...
.banner-decor {
  ...
  bottom: 0;
  width: 280px;
  background: linear-gradient(135deg, transparent 0%, rgba(180,210,240,0.5) 50%, rgba(150,190,230,0.7) 100%);
...
.dropdown-item input { accent-color: #1890ff; }
...
.filter-item .form-input { width: 140px; }
.selected-count {
  font-size: 13px;
  color: #374151;
  margin-bottom: 12px;
...
.product-check { margin-bottom: 6px; }
.product-check input { accent-color: #1890ff; }
.product-img {
  text-align: center;
  margin-bottom: 6px;
}
.img-placeholder { font-size: 32px; }
